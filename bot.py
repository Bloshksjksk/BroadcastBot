...
async def handle_broadcast(event):
    ...
    message = event.message.message[9:]
...
# Define the event handler for the /totalusers command.
@bot.on(events.NewMessage(pattern="/totalusers"))
async def handle_totalusers(event):
    # Check if the user is an admin.
    if not is_admin( event.sender_id):
        await event.reply("You are not authorized to use this command.")
        return
    ...
    # Send the total number of users to the user.
    await event.reply(f"There are {total_users} users in the database.")
